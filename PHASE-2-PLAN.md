...
| 18 | **Fixture** | Content Fixture System — dimostra le 9 entità (Article/Author/Source/Citation/Topic/Macroarea/Sotto-area/Media/Translation) — dipende da tutte le altre aree, incluso l'esito di #17 | ⏸️ Decisione presa (DR-08 Approvata: path `content/fixtures/`, Alternativa B per il topic-fixture), **implementazione rimandata a un prompt separato** — non ancora iniziata | `53b5ab0` (solo la decisione) |
...
17. **[NUOVA] Collegamento diretto Article → Media** (segnalata da DR-07, non risolta lì di proposito): DR-01 (anche esteso da DR-06) non prevede alcun campo `media` nel front matter di Article. Oggi l'unico collegamento indiretto passa per `Source` (`data/sources.yaml` ha già un campo `file:`, uso informale) o per un futuro `Dataset.media_id`. Se il Content Fixture System (DR-08, voce #18, non ancora implementata) dovrà dimostrare un collegamento Article↔Media realmente funzionante, servirà prima una futura estensione di DR-01 (stesso pattern additivo già usato da DR-06 per `sotto_area`/`temi`) — **non proposta né decisa qui**, esplicitamente rimandata al prompt che affronterà l'implementazione del Fixture System.
