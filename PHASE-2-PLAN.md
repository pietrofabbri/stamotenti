...
18. **[NUOVA] Pagine di termine taxonomy delle entità di fixture non escluse dall'indicizzazione** (trovata durante l'implementazione del Fixture System, #18 in tabella — verificata empiricamente sia in fixture isolata sia sulla build reale): il meccanismo approvato da DR-08 (cascade `sitemap.disable` + `noindex` condizionale in `seo.html`) copre solo le pagine **sotto `content/fixtures/`** (la sezione stessa e l'articolo, in entrambe le lingue) — confermato che funzionano correttamente. Non copre invece le pagine di **termine taxonomy** generate altrove nell'albero URL per le entità dedicate alla fixture: `/authors/fixture-autore/`, `/sources/fixture-fonte/`, `/temi/fixture-tema-tecnico/`, e la pagina reale `/sotto_area/sa-1-1/` (oggi popolata solo dalla fixture, dato che nessun contenuto reale usa ancora `sotto_area`). Tutte e quattro **compaiono in `sitemap.xml`** (sia IT sia EN) **senza `noindex`** — verificato con `grep` sull'output di build reale. Motivo: il cascade di Hugo si propaga alle pagine discendenti nell'albero *contenuti*, non alle pagine taxonomy generate automaticamente altrove. **Non implementata alcuna correzione in questo turno**, deliberatamente: estendere il meccanismo (es. `noindex` generico quando una pagina di termine è composta solo da pagine `content/fixtures/`) eccederebbe lo scope letterale di DR-08 così come approvato, e l'esclusione dalla sitemap per pagine taxonomy richiederebbe un template sitemap custom (non verificato, rischio/complessità maggiori del meccanismo già approvato). Segnalato per una decisione esplicita del proprietario, non risolto unilateralmente.
